...
static void init_def_termios(termios& t)
{
	t = (termios){
			ICRNL,
			OPOST | ONLCR,
			...
			case ESnormal:
				if (c>31 && c<127) {
					...
					sysbeep();
			  		...
			  		translate = (char*)GRAF_TRANS;
			  	else if (c == 15)
					translate = (char*)NORM_TRANS;
				break;
			case ESesc:
				state = ESnormal;
				switch (c)
...
void con_init(void)
{
	...
	video_num_lines = 25;//ORIG_VIDEO_LINES;
	video_page = 1;//ORIG_VIDEO_PAGE;
...
//	set_trap_gate(0x21,&keyboard_interrupt);
	CPUManager::Instance()->RegisterIRQ((IRQHandler) keyboard_interrupt,65);
